...
export async function getAnalysisHistoryPage(params: {
  ...
  const coll = db.collection("users").doc(params.userId).collection("analyses");
  let q = coll.orderBy("createdAt", "desc").limit(limit);
...
export async function getAnalysisById(analysisId: string, userId: string) {
  const db = getFirestore();
  // Prefer user-scoped subcollection (no composite indexes required).
  ...
  if (subSnap.exists) {
    ...
    if (!data) return null;
    return { id: subSnap.id, ...serializeDocData(data as Record<string, unknown>) };
  }

  // Backward-compatible fallback: legacy top-level collection (may require indexes for list queries).
  const legacySnap = await db.collection("analyses").doc(analysisId).get();
  if (!legacySnap.exists) return null;
  const legacy = legacySnap.data();
  if (!legacy) return null;
  const ownerId = (legacy as Record<string, unknown>)["userId"];
  if (typeof ownerId !== "string" || ownerId !== userId) return null;
  return { id: legacySnap.id, ...serializeDocData(legacy as Record<string, unknown>) };
}
...
export async function getUserREADMEsPage(params: {
  ...
  const coll = db.collection("users").doc(params.userId).collection("readmes");
  let q = coll.orderBy("createdAt", "desc").limit(limit);
