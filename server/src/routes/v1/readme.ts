...
export function readmeRouter() {
  ...
  router.post("/", authenticateUser, async (req, res, next) => {
    try {
      ...
      // Persist generated README for the signed-in user (all tiers).
      const owner = String(input.repo.fullName.split("/")[0] ?? "").trim();
      const tone = result.toneAnalysis ?? (await analyzeJobTone(input.job.description, input.job.url, req.log));
      const saved = await saveGeneratedREADME({
        userId: user.uid,
        githubUsername: owner || "unknown",
