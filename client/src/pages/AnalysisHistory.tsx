...
export default function AnalysisHistory() {
  ...
  const groupedAnalyses = useMemo(() => {
    const byUser = new Map<string, HistoryAnalysis[]>();
    for (const a of filteredAndSortedAnalyses) {
      const key = (a.githubUsername || "unknown").trim().toLowerCase();
    ...
    const groups = Array.from(byUser.entries()).map(([key, items]) => {
      ...
      const newest = Math.max(...items.map((i) => new Date(i.analyzedAt).getTime()));
      return { key, githubUsername: display, githubAvatar: avatar, items, newest };
    });

    // Keep groups stable and sensible: newest group first.
                      ...
                      <div className="flex items-center gap-3 px-1">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="text-xs text-muted-foreground">
                            Analyses generated for this GitHub username
                          </div>
                        </div>
                      </div>

                      <div className="space-y-4">
                        {group.items.map((analysis, index) => (
