...
import { ArrowLeft, FileText, Search, Github } from 'lucide-react';
...
export default function READMELibrary() {
    ...
    if (debouncedSearch) {
      ...
      result = result.filter(
        (r) =>
          ...
          r.githubUsername.toLowerCase().includes(query)
      );
    }
  ...
  const groupedReadmes = useMemo(() => {
    const byUser = new Map<string, SavedREADME[]>();
    for (const r of filteredAndSortedReadmes) {
      const key = (r.githubUsername || "unknown").trim().toLowerCase();
      const next = byUser.get(key) ?? [];
      next.push(r);
      byUser.set(key, next);
    }

    const groups = Array.from(byUser.entries()).map(([key, items]) => {
      const display = items[0]?.githubUsername || key || "unknown";
      const avatar = items[0]?.githubAvatar || (display ? `https://github.com/${display}.png` : "");
      const newest = Math.max(...items.map((i) => new Date(i.generatedAt).getTime()));
      return { key, githubUsername: display, githubAvatar: avatar, items, newest };
    });

    groups.sort((a, b) => b.newest - a.newest || a.githubUsername.localeCompare(b.githubUsername));
    return groups;
  }, [filteredAndSortedReadmes]);
        ...
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          {isLoading ? (
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="space-y-6">
              {groupedReadmes.map((group) => {
                const fallback = (group.githubUsername || "?").slice(0, 2).toUpperCase();
                return (
                  <div key={group.key} className="space-y-3">
                    <div className="flex items-center gap-3 px-1">
                      <Avatar className="h-9 w-9 border border-border">
                        <AvatarImage src={group.githubAvatar} alt={`${group.githubUsername} avatar`} />
                        <AvatarFallback className="text-[10px] font-medium">{fallback}</AvatarFallback>
                      </Avatar>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          <Github className="w-4 h-4 text-muted-foreground" />
                          <span className="font-semibold text-foreground truncate">{group.githubUsername}</span>
                          <span className="text-xs text-muted-foreground">({group.items.length})</span>
                        </div>
                        <div className="text-xs text-muted-foreground">
                          Generated READMEs for repos owned by this GitHub username
                        </div>
                      </div>
                    </div>

                    <div className="space-y-4">
                      {group.items.map((readme, index) => (
                        <READMECard key={readme.id} readme={readme} index={index} onViewFull={handleViewFull} />
                      ))}
                    </div>
                  </div>
                );
